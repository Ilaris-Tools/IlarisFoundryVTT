...
- ActiveEffect and supernatural Pre-Effect configuration distinguish classic Foundry changes from Ilaris modifiers. Generated data, LLM-assisted Pre-Effect generation, and every spell listed in `spell-liturgy-effect-inventory.md` are migrated in the first iteration; Axxeleratus is the reference example.
