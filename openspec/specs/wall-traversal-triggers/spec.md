...
contains an ENTER segment for that Region. It SHALL not infer traversal from
final Region membership alone. Teleport movement, initial occupancy, direct
document repositioning, Region-boundary edits, MOVE-only paths, and EXIT-only
...
- **THEN** its processed Region path SHALL contain an ENTER segment
