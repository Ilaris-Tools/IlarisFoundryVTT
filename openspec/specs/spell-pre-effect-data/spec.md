...
Heal spells in the compendium SHALL include pre-effect configurations that heal wounds.

#### Scenario: Balsam heals 2W6+4 wounds

- **WHEN** a GM casts Balsam and the spell succeeds
- **THEN** the target SHALL receive `-2W6-4` (negated heal formula) via `_applyDamageDirectly`, which reduces wounds by WS thresholds
