...
// Cross-platform Foundry lifecycle facade for local and credential-gated remote E2E validation.
...
function parseOptions(args) {
    const options = { action: args[0] || 'Status', mode: 'local', world: 'ilaris-e2e-world-v14363-r1', port: 30000, timeout: 60 }
    for (let index = 1; index < args.length; index += 1) {
        const option = args[index]
        const value = args[index + 1]
        if (option === '--world') options.world = value
        if (option === '--mode') options.mode = value
        if (option === '--home') options.home = value
        if (option === '--port') options.port = Number(value)
        if (option === '--timeout') options.timeout = Number(value)
        if (option.startsWith('--')) index += 1
    }
    const localActions = ['Status', 'Stop', 'Pack', 'Start', 'Restart', 'PackAndRestart']
    const remoteActions = ['Setup', 'Status', 'Stop', 'Start', 'Restart', 'Logs', 'Reset', 'Share', 'Unshare']
    if (!['local', 'remote'].includes(options.mode)) throw new Error("Mode must be either 'local' or 'remote'.")
    if (!(options.mode === 'local' ? localActions : remoteActions).includes(options.action)) {
...
if (options.mode === 'remote') {
    const { parseRemoteOptions, runRemoteLifecycle } = await import('./foundry-env/remote-lifecycle.mjs')
    const forwarded = [options.action, '--port', String(options.port), '--timeout', String(options.timeout)]
    if (options.home) forwarded.push('--home', options.home)
    const result = await runRemoteLifecycle(parseRemoteOptions(forwarded))
