...
async function prepareApplication(home, environment, version) {
    ...
    if (existsSync(marker) && readFileSync(marker, 'utf8').trim() === String(version))
        return findMain(appDirectory)
    const release = path.join(home, 'cache', `foundryvtt-${version}.zip`)
    const result = spawnSync(
        process.execPath,
        [path.join(root, 'utils/foundry-env/download-foundry.mjs'), version, release],
        {
            cwd: root,
            env: environment,
            stdio: 'inherit',
    ...
    if (result.status !== 0)
        throw new Error(`Foundry application download failed with exit code ${result.status}.`)
    ...
    if (extraction.error?.code === 'ENOENT')
        throw new Error('Could not extract Foundry: install unzip or use Windows PowerShell.')
    if (extraction.status !== 0)
        throw new Error('Could not extract the Foundry application archive.')
...
function ensureCompendiumPacks() {
    ...
    const invocation = commandForPlatform(process.platform === 'win32' ? 'npm.cmd' : 'npm', [
        'run',
        'pack-all',
    ])
...
export async function stopOwnedProcess(home, name = 'foundry') {
    ...
    if (processIsRunning(owned.pid)) {
        ...
        if (processIsRunning(owned.pid))
            throw new Error(`Managed ${name} PID ${owned.pid} did not stop.`)
    }
...
function writeServerConfig(dataRoot, port, manifest, environment) {
    ...
    writeFileSync(
        ...
        JSON.stringify({
            license: String(environment.FOUNDRY_LICENSE_KEY).replace(/[^A-Za-z0-9]/g, ''),
        }),
    )
    writeFileSync(
        path.join(dataRoot, 'Config', 'options.json'),
        JSON.stringify(
            { port, upnp: false, world: manifest.worldId, language: 'en.core' },
            null,
            2,
        ),
    )
...
export async function setupRemote({
    ...
    if (missing.length)
        return {
            ...
            message: `Missing ${missing.join('; ')}. Configure them in the process environment or ${secretPath}.`,
        }

    const manifest = await readBaselineManifest(
        path.join(root, 'e2e', 'fixtures', 'baselines', 'manifest.json'),
    )
    ...
    if (!existsSync(path.join(worldDirectory, 'world.json'))) {
        ...
        if (result.error?.code === 'ENOENT')
            throw new Error('Could not extract baseline: install unzip or use Windows PowerShell.')
        if (result.status !== 0)
            throw new Error('Could not extract the verified E2E baseline archive.')
    }
    ...
    if (!existsSync(systemLink))
        symlinkSync(root, systemLink, process.platform === 'win32' ? 'junction' : 'dir')
    writeServerConfig(dataRoot, port, manifest, resolvedEnvironment)
    writeFileSync(
        path.join(resolvedHome, 'remote.json'),
        JSON.stringify({ port, world: manifest.worldId, dataRoot }, null, 2),
    )
    return {
        exitCode: 0,
        message: `Prepared ${manifest.worldId} at ${resolvedHome}.`,
        manifest,
...
export async function startRemote(options = {}) {
    ...
    if (await isReady(remote.port))
        return {
            exitCode: 0,
            message: `Foundry is already ready on http://127.0.0.1:${remote.port}.`,
        }
    const prior = readOwnedProcess(setup.home)
    if (prior?.pid && processIsRunning(prior.pid))
        throw new Error(
            `Managed Foundry PID ${prior.pid} is running but not ready; inspect ${path.join(setup.home, 'foundry.log')}.`,
        )
    if (prior) unlinkSync(prior.file)
    const main = await prepareApplication(
        setup.home,
        setup.environment,
        setup.manifest.foundryVersion,
    )
    const log = path.join(setup.home, 'foundry.log')
    const descriptor = openSync(log, 'a')
    const child = spawn(
        process.execPath,
        [main, `--dataPath=${remote.dataRoot}`, `--port=${remote.port}`, '--noupnp'],
        {
            detached: true,
    ...
    return {
        exitCode: 0,
        message: `Foundry is ready on http://127.0.0.1:${remote.port} (world: ${setup.manifest.worldId}).`,
    }
}
...
async function resolveCloudflared(home) {
    if (process.env.FOUNDRY_CLOUDFLARED) return process.env.FOUNDRY_CLOUDFLARED
    if (process.platform === 'win32') return 'cloudflared.exe'
    const managed = path.join(home, 'bin', 'cloudflared')
    if (existsSync(managed)) return managed
    if (spawnSync('cloudflared', ['--version'], { stdio: 'ignore' }).status === 0)
        return 'cloudflared'
    // Agent containers usually have no cloudflared; fetch the static binary once.
    const assets = { x64: 'cloudflared-linux-amd64', arm64: 'cloudflared-linux-arm64' }
    const asset = assets[process.arch]
    if (!asset || process.platform !== 'linux')
        throw new Error('Could not find cloudflared. Install cloudflared and retry.')
...
async function shareRemote(home, port) {
    if (!(await isReady(port)))
        throw new Error(`Foundry is not running on port ${port}; start it before sharing.`)
    console.warn(
        'SECURITY WARNING: this test server has an unauthenticated GM. Anyone with the public URL can join as GM.',
    )
    const existing = readOwnedProcess(home, 'tunnel')
    const log = path.join(home, 'tunnel.log')
    if (existing && processIsRunning(existing.pid))
        return { exitCode: 0, message: `Tunnel is already running. ${logTail(home, 20)}` }
    const command = await resolveCloudflared(home)
    ...
    if (child.pid === undefined)
        throw new Error('Could not start cloudflared. Install cloudflared and retry.')
    child.unref()
    writeFileSync(path.join(home, 'tunnel.pid'), String(child.pid))
    await wait(1000)
    if (!processIsRunning(child.pid))
        throw new Error(`cloudflared stopped immediately. ${logTail(home, 20)}`)
...
async function resetRemote(home) {
    if (!isManagedRemoteHome(home, defaultHome)) {
        throw new Error(
            `Refusing to reset '${home}': only the default managed remote home '${defaultHome}' may be deleted.`,
        )
    }
...
export async function runRemoteLifecycle(options) {
    ...
    if (action === 'Stop')
        return {
            exitCode: 0,
            message: (await stopOwnedProcess(home))
                ? 'Stopped managed remote Foundry.'
                : 'Managed remote Foundry is not running.',
        }
    if (action === 'Restart') {
    ...
    if (action === 'Status')
        return {
            exitCode: (await isReady(port)) ? 0 : 1,
            message: (await isReady(port))
                ? `Foundry is ready on http://127.0.0.1:${port}.`
                : `Foundry is not reachable on http://127.0.0.1:${port}.`,
        }
    ...
    return {
        exitCode: 0,
        message: (await stopOwnedProcess(home, 'tunnel'))
            ? 'Tunnel stopped; the public URL is no longer available.'
            : 'No managed tunnel is running.',
    }
}
