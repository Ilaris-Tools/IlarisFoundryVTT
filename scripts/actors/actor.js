...
export class IlarisActor extends Actor {
    ...
    _checkVorteilSource(requirement, vorteil) {
        return (
            (vorteil.flags?.core?.sourceId && vorteil.flags.core.sourceId.includes(requirement)) ||
            (vorteil._stats.compendiumSource &&
                vorteil._stats.compendiumSource.includes(requirement))
        )
    }
    ...
    _hasVorteil(vorteilRequirements) {
        ...
        return (
            this.vorteil.allgemein.some((vorteil) => {
                return vorteilRequirements.some((requirement) =>
    ...
    _hasKampfstilSelected(stilRequirements) {
        return (
            stilRequirements.some((requirement) =>
                hardcoded
                    .getSelectedStil(this.system.misc.selected_kampfstil, this.misc.kampfstile_list)
                    .sources.some((source) => source.includes(requirement)),
            ) ||
            stilRequirements.some((requirement) =>
                hardcoded
                    .getSelectedStil(
                        this.system.misc.selected_uebernatuerlicher_stil,
                        this.misc.uebernatuerliche_stile_list,
                    )
                    .sources.some((source) => source.includes(requirement)),
            )
        )
    ...
    async _calculateKampf(actor) {
        ...
        let selected_kampfstil = hardcoded.getSelectedStil(
        ...
        let selected_uebernatuerlicher_stil = hardcoded.getSelectedStil(
            actor.system.misc?.selected_uebernatuerlicher_stil ?? 'ohne',
            uebernatuerliche_stile,
        )
